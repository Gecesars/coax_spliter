...
class RFCalculator(ABC):
    ...
    def plot_performance(self):
        ...
        plt.tight_layout()
        return fig
def interactive_plot_performance(frequencies, gamma_th, network_sim, N):
    """
    Retorna uma Figure com 3 subplots interativos:
     - Return Loss (S11) dB
     - Insertion Loss (S(n,1)) dB
     - Smith Chart (S11)
    """
    import matplotlib.pyplot as plt
    import numpy as np
    ...
    # Insertion Loss teórico
    il_db_th = -20*np.log10(np.clip(np.abs(1-gamma_th)/np.sqrt(N), eps, None))
    ...
    if network_sim:
        rl_db_sim = -20*np.log10(np.clip(np.abs(network_sim.s[:,0,0]), eps, None))
        ax1.plot(network_sim.frequency.f/1e6, rl_db_sim, '--', label="S11 Simulado")
    ax1.set(title="Return Loss (S11)", xlabel="MHz", ylabel="dB")
    ax1.set_ylim(-60,0)
    ax1.grid(True)
    ax1.legend()
    ...
    for i in range(N):
        port = i+2
        ax2.plot(f_mhz, il_db_th, color=colors[i], linestyle='-', label=f"S{port}1 Teórico")
        if network_sim and network_sim.s.shape[1]>i+1:
            il_db_sim = -20*np.log10(np.clip(np.abs(network_sim.s[:,i+1,0]), eps, None))
            ax2.plot(network_sim.frequency.f/1e6, il_db_sim, '--', color=colors[i], label=f"S{port}1 Simulado")
    ax2.set(title="Insertion Loss (S(n,1))", xlabel="MHz", ylabel="dB")
    ax2.set_ylim(-6,0)
    ax2.grid(True)
    ax2.legend()
    ...
    plot_s_smith(ntw_th, ax=ax3, show_legend=False)
    if network_sim:
        plot_s_smith(network_sim, ax=ax3, show_legend=False, linestyle='--')
    ax3.set_title("Smith Chart (S11)")
    ...
    def on_scroll(event):
        ...
        ax.set_xlim(xdata-new_w*(xdata-x0)/(x1-x0), xdata+new_w*(x1-xdata)/(x1-x0))
        ax.set_ylim(ydata-new_h*(ydata-y0)/(y1-y0), ydata+new_h*(y1-ydata)/(y1-y0))
