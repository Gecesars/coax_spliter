...
class RFCalculator(ABC):
    ...
    def theoretical_return_loss(self):
        ...
        Zs = [z0 * (z_eff / z0) ** ((2 * i - 1) / (2 * p['n_sections'])) for i in range(1, p['n_sections'] + 1)]
        ntw = None
        for Z in Zs:
            media = DistributedCircuit(frequency=fr, z0=Z)
            seg = media.line(sec_len_m, unit='m')
            ntw = seg if ntw is None else ntw ** seg
        load = DistributedCircuit(frequency=fr, z0=z_eff).resistor(z_eff)
        full_network = ntw ** load.inv
        return full_network.s[:, 0, 0]
    ...
    def plot_performance(self):
        ...
        fig.suptitle("Análise de Desempenho Comparativa (Teórico vs. Simulado)", fontsize=16)
        ...
        if self.s_params_th:
            ...
            ntw_th.plot_s_smith(m=0, n=0, ax=ax3, show_legend=False, chart_style='fancy', color='blue', label='Teórico')
        if self.s_params_sim:
...
class CoaxialCalculator(RFCalculator):
    ...
    def calculate_s_parameters(self):
        ...
        Gamma = s11
        mismatch_loss_db = -10 * np.log10(np.abs(1 - np.abs(Gamma) ** 2))
...
# --------------------------------------------------------------------
def add_dimension(plotter, p1, p2, label, offset_dir, is_2d=False, text_size=16):
    """Função auxiliar para adicionar cotas limpas a uma vista 2D ou 3D."""
    p1, p2, offset_dir = np.array(p1), np.array(p2), np.array(offset_dir)
    dist = np.linalg.norm(p1 - p2)
    if dist < 1e-6: return
    mid_point = (p1 + p2) / 2
    offset_vec = offset_dir / np.linalg.norm(offset_dir)
    offset_dist = (dist * 0.15 + 8) if not is_2d else 12
    ext_p1 = p1 + offset_vec * (offset_dist * 0.8)
    ext_p2 = p2 + offset_vec * (offset_dist * 0.8)
    ...
    label_text = f"{dist:.2f}" if isinstance(dist, (int, float)) else label
    label_pos = mid_point + offset_vec * (offset_dist * 1.1)
    plotter.add_point_labels([label_pos], [label_text], font_size=text_size, text_color='black' if is_2d else '#00FFFF',
...
def create_coaxial_geometry(p, for_report=False):
    ...
    plotter.add_mesh(diel_principal, color='#D3D3D3', opacity=0.1)
    z_pos = 0
...
def create_coaxial_geometry_2d_side_view(p):
    ...
    for di in p['main_diams']:
        ...
        z_pos += sec_len
    add_dimension_2d(plotter, [0, -diel_radius, 0], [diel_height, -diel_radius, 0], f"L Total: {diel_height:.2f} mm",
                     [0, -1, 0], text_size=14)
    add_dimension_2d(plotter, [0, -diel_radius, 0], [0, diel_radius, 0], f"D Tubo: {p['d_int_tube']:.2f}", [-1, 0, 0],
                     text_size=14)
    z_pos = 0
...
def export_full_pdf(calc: RFCalculator, queue: queue.Queue):
    """Gera um relatório PDF completo com múltiplas vistas, gráficos e tabelas detalhadas."""
    ...
    with tempfile.TemporaryDirectory() as tempdir:
        ...
        story.append(Paragraph("Análise de Desempenho (Teórico vs. Simulado)", styles['h2']))
        story.append(RLImage(img_perf_path, width=9.8 * inch, height=3.2 * inch));
        story.append(PageBreak())
        story.append(Paragraph("Folha de Dados do Projeto", styles['h2']));
        story.append(Spacer(1, 12))
        ...
        for i, diam in enumerate(r['main_diams'], 1): mech_data.append([f"Diâmetro Seção {i}", f"{diam:.3f}"])
        elec_data = [["Parâmetros Elétricos (Ω)", "Valor"]]
        for i, z in enumerate(r['z_sects'], 1): elec_data.append([f"Impedância Seção {i}", f"{z:.2f}"])
        ...
        t1 = Table(param_data, colWidths=[2.5 * inch, 2 * inch], hAlign='LEFT');
        t1.setStyle(tbl_style)
        t2 = Table(mech_data, colWidths=[2.5 * inch, 2 * inch], hAlign='LEFT');
        t2.setStyle(tbl_style)
        t3 = Table(elec_data, colWidths=[2.5 * inch, 2 * inch], hAlign='LEFT');
...
# --------------------------------------------------------------------
# 8. Função de Geração HFSS (estável e corrigida)
# --------------------------------------------------------------------
def generate_hfss_model(params, status_queue):
    ...
    try:
        ...
        with Hfss(projectname=project_path, designname="DivisorCoaxial", solution_type="Modal", new_desktop=True,
            ...
            num_saidas = int(p['n_outputs'])
            if num_saidas > 6:
                vm["dia_saida_diel"] = "dia_int_tubo / 3"; vm["dia_saida_cond"] = "(dia_int_tubo / 3) / 2.3"
            elif num_saidas > 4:
                vm["dia_saida_diel"] = "dia_int_tubo / 2.2"; vm["dia_saida_cond"] = "(dia_int_tubo / 2.2) / 2.3"
            else:
                vm["dia_saida_diel"] = "dia_int_tubo"; vm["dia_saida_cond"] = f"{p['d_out_50ohm']}mm"
            ...
            if p['n_outputs'] > 1: mdl.duplicate_around_axis([output_outer_1.name, output_inner_1.name], axis="Z",
            ...
            output_inners_names = [name for name in mdl.object_names if name.startswith("Output_Inner_")];
            ...
            if mat_name not in hfss.materials: new_mat = hfss.materials.add_material(
                mat_name); new_mat.permittivity = er_val; new_mat.dielectric_loss_tangent = tand
            final_diel_obj.material_name = mat_name;
            final_cond_obj.material_name = "pec"
            for face in final_diel_obj.faces:
                if abs(face.center[2]) < 1e-6: hfss.wave_port(assignment=face.id, name="P1", renormalize=True,
                                                              impedance="50ohm"); break
            for k in range(p['n_outputs']):
                ...
                for face in final_diel_obj.faces:
                    center = face.center
                    if np.linalg.norm(np.array(center) - np.array([pos_x, pos_y, pos_z])) < 1e-3: hfss.wave_port(
                        assignment=face.id, name=f"P{k + 2}", renormalize=True, impedance="50ohm"); break
...
# --------------------------------------------------------------------
# 9. GUI CTk (Classes App, ParameterFrame, ResultFrame)
# --------------------------------------------------------------------
class App(ctk.CTk):
    def __init__(self):
        super().__init__()
    ...
    def calculate_and_display(self, params):
        ...
        if self.calc.calculate():
            ...
            messagebox.showerror("Erro de Cálculo", "Não foi possível calcular com os parâmetros fornecidos.")
            self.calc = None
            self.update_button_states()
    ...
    def _run_hfss_simulation_thread(self):
        if not HFSS_AVAILABLE or not self.project_path:
            self.queue.put("ERRO: Simulação HFSS não pode ser iniciada.")
            return
        try:
            self.queue.put("Iniciando simulação HFSS...")
            with Hfss(project=self.project_path, new_desktop=False, close_on_exit=True) as hfss:
                self.queue.put("Analisando o setup 'Setup1'...")
                hfss.analyze_setup("Setup1")
                self.queue.put("Simulação HFSS concluída.")
                export_dir = os.path.dirname(self.project_path)
                touchstone_file = hfss.export_touchstone("Setup1", "Sweep1", export_dir)
                self.queue.put(f"Resultados exportados para: {touchstone_file[0]}")
                self.after(0, lambda: self.import_hfss_results(touchstone_file[0]))
        except Exception as e:
            self.queue.put(f"ERRO na simulação HFSS: {e}")
            traceback.print_exc()

    def run_simulation(self):
        if self.project_path:
            thread = threading.Thread(target=self._run_hfss_simulation_thread, daemon=True)
            thread.start()

    def import_hfss_results(self, filepath=None):
        if not self.calc:
            messagebox.showwarning("Aviso", "Calcule um projeto primeiro.");
            return
        if not filepath:
            filepath = fd.askopenfilename(title="Selecione o arquivo Touchstone (.sNp)",
...
class ParameterFrame(ctk.CTkFrame):
    def __init__(self, parent, app):
        ...
        ctk.CTkLabel(self, text="Parâmetros de Entrada", font=("Arial", 16, "bold")).grid(row=0, column=0, columnspan=2,
                                                                                          pady=10)
        self.entries = {}
        params_layout = {"Freq. Inicial (MHz):": ("f_start", "800"), "Freq. Final (MHz):": ("f_stop", "1200"),
    ...
    def _validate_and_calc(self):
        try:
            ...
            if params['f_stop'] <= params['f_start']: raise ValueError("Frequência final deve ser maior que a inicial.")
            if params['d_ext'] <= 2 * params['wall_thick']: raise ValueError(
                "Diâmetro externo deve ser maior que o dobro da espessura da parede.")
            if params['n_sections'] <= 0 or params['n_outputs'] <= 0: raise ValueError(
                "Número de seções e saídas devem ser inteiros positivos.")
...
class ResultFrame(ctk.CTkFrame):
    ...
    def display_results(self, calc: RFCalculator):
        ...
        for widget in self.geo_frame.winfo_children(): widget.destroy()
        text_area = ctk.CTkTextbox(self.geo_frame, font=("Courier New", 12));
        text_area.pack(fill="both", expand=True, padx=10, pady=10)
        p, r = calc.params, calc.results
        report = f"""--- Parâmetros Geométricos e Elétricos ---

{'Parâmetro':<30} | {'Valor'}
{'-' * 50}
{'Comprimento de Seção (mm)':<30} | {r['sec_len_mm']:.3f}
{'Comprimento Interno Total (mm)':<30} | {r['len_inner_mm']:.3f}
{'Diâmetro Interno do Tubo (mm)':<30} | {r['d_int_tube']:.3f}
{'Diâmetro Saída 50 Ohm (mm)':<30} | {r['d_out_50ohm']:.3f}
