...
class RFCalculator(ABC):
    ...
    def plot_performance(self):
        """
        Gera 3 gráficos comparando Teoria vs Simulação, com ondulação realista:
          1) Return Loss (S11) em dB
          2) Insertion Loss (S(n,1)) em dB
        ...
        # 1) Cálculo de Γ(f) teórico
        gamma_th = self.theoretical_return_loss()

        # 2) Return Loss teórico
        rl_db_th = -20 * np.log10(np.clip(np.abs(gamma_th), eps, None))

        # 3) Insertion Loss teórico via (1 - Γ)/√N
        s21_lin_th = np.abs(1 - gamma_th) / np.sqrt(N)
        il_db_th = -20 * np.log10(np.clip(s21_lin_th, eps, None))

        # --- Cria figura ---
        fig = plt.figure(figsize=(18, 5))
        gs = fig.add_gridspec(1, 3)

        # --- Gráfico 1: Return Loss (S11) ---
        ax1 = fig.add_subplot(gs[0, 0])
        ax1.plot(f_mhz, rl_db_th, '-', label="S11 Teórico")
        if self.s_params_sim:
            rl_db_sim = -20 * np.log10(np.clip(np.abs(self.s_params_sim.s[:, 0, 0]), eps, None))
            ax1.plot(self.s_params_sim.frequency.f / 1e6, rl_db_sim, '--', label="S11 Simulado")
        ax1.set(title="Return Loss (S11)", xlabel="MHz", ylabel="dB")
        ax1.set_ylim(-60, 0)
        ax1.grid(True)
        ax1.legend()

        # --- Gráfico 2: Insertion Loss (S(n,1)) ---
        ax2 = fig.add_subplot(gs[0, 1])
        colors = plt.cm.viridis(np.linspace(0, 1, N))
        for i in range(N):
            port = i + 2
            # teórico
            ax2.plot(f_mhz, il_db_th, color=colors[i], linestyle='-',
                     label=f"S{port}1 Teórico")
            # simulado, se existir
            if self.s_params_sim and self.s_params_sim.s.shape[1] > i + 1:
                il_db_sim = -20 * np.log10(np.clip(
                    np.abs(self.s_params_sim.s[:, i + 1, 0]), eps, None))
                ax2.plot(self.s_params_sim.frequency.f / 1e6, il_db_sim,
                         color=colors[i], linestyle='--', label=f"S{port}1 Simulado")
        ax2.set(title="Insertion Loss (S(n,1))", xlabel="MHz", ylabel="dB")
        ax2.set_ylim(-6, 0)  # cerca de -3 dB com ondulação
        ax2.grid(True)
        ax2.legend()
        ...
        # teórico
        ntw_th = rf.Network()
        ntw_th.frequency = Frequency.from_f(f_hz, unit='Hz')
        s_temp = np.zeros((len(f_hz), 2, 2), dtype=complex)
        s_temp[:, 0, 0] = gamma_th
        ntw_th.s = s_temp
        plot_s_smith(ntw_th, ax=ax3, show_legend=False)
        # simulado
        if self.s_params_sim:
            plot_s_smith(self.s_params_sim, ax=ax3,
                         show_legend=False, linestyle='--')
